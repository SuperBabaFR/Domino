...
def domino_playable(domino, table_de_jeu, side):
    ...
    if (side == "left" and value_left not in [domino.left, domino.right]) or (side == "right" and value_right not in [domino.left, domino.right]):
        return False
...
class CreateGame(APIView):
    ...
    def post(self, request):
        ...
        for player_id in player_id_list:
            ...
            HandPlayer.objects.create(round=round, session=session, player=Player.objects.get(id=player_id), dominoes=dominoes)
...
class PlaceDomino(APIView):
    ...
    def post(self, request):
        ...
        if len(table_de_jeu) == 0 and max(player_dominoes) != domino.id:
            return Response(dict(code=400, message="Le joueur doit jouer son plus gros domino", data=None), status=status.HTTP_400_BAD_REQUEST)
        ...
        if len(table_de_jeu) > 0 and playable_values == False:
            return Response(dict(code=400, message="Domino non jouable", data=None), status=status.HTTP_400_BAD_REQUEST) # Domino non jouable
        ...
        if domino.left != domino.right:
            if (side == "left" and domino.left == playable_values["left"]) or (side == "right" and domino.right == playable_values["right"]):
                orientation = "inverse"
        else:
            orientation = "double"
